...
async function toBadgeDefinition(t, e, n, i, s, r) {
    t.kind = KnownEventKind.BADGE_DEFINATION, t.tags.push([ "d", e ]), t.tags.push([ "name", n ]), 
    t.tags.push([ "description", i ]), t.tags.push(s);
    for (const a of r) t.tags.push(a);
}
...
class Channel {
    ...
    static from(t, e) {
        ...
        let s = void 0;
        var e = ((s = e ? e.filter(t => {
            return t.pubkey === n && t.tags.find(t => "e" === t[0])?.[1] === i;
        }).sort((t, e) => 0 <= t.created_at - e.created_at ? -1 : 1)[0] : s) || t).content, [ , , r ] = s ? s.tags.find(t => "e" === t[0]) : [];
        const {
            name: a,
            about: o,
            picture: c,
...
class ChannelMessage extends BaseEvent {
    ...
    replyTo;
    static from(t) {
        if (t.kind !== Kind.ChannelMessage) throw new Error(`Must be an kind-${Kind.ChannelMessage}: channel message`);
        var e = t.tags.find(t => "e" === t[0] && "root" === t[3]), n = e && e[1], e = e && e[2], i = t.tags.find(t => "e" === t[0] && "reply" === t[3]), s = i && i[1], i = i ? i[2] : "", r = t.tags.find(t => "p" === t[0]), r = r && r[1];
...
async function fetchContactsLikeMe(t, e) {
    e = {
        kinds: [ KnownEventKind.CONTACT ],
        "#p": [ e ]
    };
    return (await t.fetch([ e ])).map(t => Contact.from(t));
}
